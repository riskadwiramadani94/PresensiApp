...
export default function NotificationScreen() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const role = await AsyncStorage.getItem('userRole') || 'pegawai';
      ...
      console.log('[NOTIF] Fetching notifications for user:', userId, 'role:', role);
      ...
      if (data.success && data.data) {
        ...
        setNotifications([]);
        console.log('[NOTIF] No notifications or error:', data.message);
      }
    } catch (error) {
  ...
  const handleNotificationPress = (notification: PushNotification) => {
    console.log('[NOTIF] Notification pressed:', notification);
    
    const { type, reference_type, reference_id } = notification;
    
    // Route berdasarkan type notifikasi
    switch (type) {
      // === NOTIFIKASI UNTUK PEGAWAI ===
      
      case 'pengajuan_approved':
      case 'pengajuan_rejected':
        // Pegawai ke halaman pengajuan
        router.push('/menu-pegawai/pengajuan');
        break;
      
      case 'dinas_assigned':
        // Pegawai ke halaman kegiatan
        router.push('/menu-pegawai/kegiatan');
        break;
      
      case 'lembur_approved':
      case 'lembur_rejected':
        // Pegawai ke halaman lembur
        router.push('/menu-pegawai/lembur');
        break;
      ...
      // === FALLBACK ===
      default:
        console.log('[NOTIF] Unknown notification type:', type);
        ...
        if (reference_type === 'pengajuan') {
          ...
          if (userRole === 'admin') {
            ...
            router.push('/menu-pegawai/presensi');
          }
        }
        break;
    }
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtext}>
                Notifikasi akan muncul di sini saat ada update terbaru
              </Text>
            </View>
          )}
        />
