...
export default function EditDinasScreen() {
  ...
  const reverseGeocode = async (latitude: number, longitude: number) => {
    try {
      ...
      if (result.length > 0) {
        ...
        const fullAddress =
          `${address.street || ""} ${address.name || ""}, ${address.district || ""}, ${address.city || ""}, ${address.region || ""} ${address.postalCode || ""}`.trim();
        return fullAddress;
      }
    } catch (error) {
  ...
  const fetchDinasData = async () => {
    try {
      ...
      if (response.success && response.data) {
        const dinasData = response.data.find(
        ...
        if (dinasData) {
          setFormData({
            namaKegiatan:
              dinasData.nama_kegiatan || dinasData.namaKegiatan || "",
            nomorSpt: dinasData.nomor_spt || dinasData.nomorSpt || "",
            jenisDinas:
              dinasData.jenis_dinas || dinasData.jenisDinas || "lokal",
          ...
          if (dinasData.pegawai) {
            ...
            if (pegawaiResponse.success && pegawaiResponse.data) {
              ...
              setFormData((prev) => ({
                ...prev,
                pegawaiIds: selectedPeg.map(
                  (p: any) => p.id_user || p.id_pegawai || p.id,
                ),
              }));
            }
          }
    ...
    switch (field) {
      ...
      case "tanggalSelesai":
        if (!value) {
          ...
          new Date(convertDateFormat(value)) <
            new Date(convertDateFormat(formData.tanggalMulai))
        ) {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedLokasi.length === 0)
      errors.lokasi = "Minimal pilih 1 lokasi untuk dinas";
    if (selectedPegawai.length === 0)
      errors.pegawai = "Minimal pilih 1 pegawai untuk dinas";
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append(
        "tanggal_mulai",
        convertDateFormat(formData.tanggalMulai),
      );
      formDataToSend.append(
        "tanggal_selesai",
        convertDateFormat(formData.tanggalSelesai),
      );
      formDataToSend.append(
        "jam_mulai",
        useDefaultJam ? "" : formData.jamMulai,
      );
      formDataToSend.append(
        "jam_selesai",
        useDefaultJam ? "" : formData.jamSelesai,
      );
      formDataToSend.append("deskripsi", formData.deskripsi?.trim() || "");

      const validPegawaiIds = formData.pegawaiIds.filter(
        (id) => id != null && !isNaN(id) && id > 0,
      );
      formDataToSend.append("pegawai_ids", JSON.stringify(validPegawaiIds));
      formDataToSend.append(
        "lokasi_ids",
        JSON.stringify(selectedLokasi.map((lokasi) => lokasi.id)),
      );

      if (
        selectedFile &&
        selectedFile.uri &&
        !selectedFile.uri.includes("SPT-")
      ) {
          ...
          <View style={styles.formContent}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={[
                  styles.textInput,
                  validationErrors.namaKegiatan && styles.inputError,
                ]}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={[
                  styles.textInput,
                  validationErrors.nomorSpt && styles.inputError,
                ]}
          ...
          <View style={styles.formContent}>
                ...
                <View style={styles.dropdownContainer}>
                  ...
                  <ScrollView style={styles.dropdownList} nestedScrollEnabled>
                    {filteredLokasi.map((lokasi) => {
                      const isSelected = selectedLokasi.find(
                        (l) => l.id === lokasi.id,
                      );
                      return (
                        <TouchableOpacity
                ...
                <View style={styles.selectedContainer}>
                  {selectedLokasi.map((lokasi) => (
                    <View key={lokasi.id} style={styles.selectedChip}>
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          const updated = selectedLokasi.filter(
                            (l) => l.id !== lokasi.id,
                          );
            ...
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>
                Pegawai Dinas * ({selectedPegawai.length} dipilih)
              </Text>
              <TouchableOpacity
                style={styles.dropdownBtn}
                ...
                <View style={styles.dropdownContainer}>
                  ...
                  <ScrollView style={styles.dropdownList} nestedScrollEnabled>
                    {filteredPegawai.map((pegawai: any, index) => {
                      const pegawaiId =
                        pegawai.id_user || pegawai.id_pegawai || pegawai.id;
                      const isSelected = selectedPegawai.find((p: any) => {
                        const selectedId = p.id_user || p.id_pegawai || p.id;
                        return selectedId === pegawaiId;
                      });
            ...
            <ScrollView style={styles.confirmModalContent}>
              ...
              <View style={styles.confirmItem}>
                <Text style={styles.confirmLabel}>
                  Lokasi ({selectedLokasi.length}):
                </Text>
                {selectedLokasi.map((lokasi, index) => (
                  <Text key={index} style={styles.confirmValue}>
                    • {lokasi.nama_lokasi}
                  </Text>
                ))}
              </View>
              ...
              <View style={styles.confirmItem}>
                <Text style={styles.confirmLabel}>
                  Pegawai ({selectedPegawai.length}):
                </Text>
                  ...
                  Alert.prompt("Nama Kegiatan", "Masukkan nama kegiatan:", [
                    { text: "Batal", style: "cancel" },
                    {
                      ...
                      onPress: (value: string | undefined) =>
                        value &&
                        setFormData({ ...formData, namaKegiatan: value }),
                    },
                  ]);
                }}
              >
                ...
                onPress={() => {
                  Alert.prompt("Nomor SPT", "Masukkan nomor SPT:", [
                    { text: "Batal", style: "cancel" },
                    {
                      text: "OK",
                      onPress: (value: string | undefined) =>
                        value && setFormData({ ...formData, nomorSpt: value }),
                    },
                  ]);
                }}
              >
